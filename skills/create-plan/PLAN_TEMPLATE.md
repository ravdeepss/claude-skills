...
- **Model routing config:** {`model-config.json` loaded from {location} | no config — using generic defaults}
...
- `model-config.json` — model routing config: tier→model mapping, role-based routing, budget constraints (used by plan-runner for dispatch decisions) *(include only if the file exists — use the resolved path from the lookup hierarchy)*
...
**Tier:** capable
**Suggested model:** {resolved from model-config.json if available; otherwise e.g., "Sonnet, GLM-5.1"}
